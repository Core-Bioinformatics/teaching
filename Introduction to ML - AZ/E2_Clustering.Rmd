...
```{r clusterTypes, fig.cap='Example clusters. **A**, *blobs*; **B**, *aggregation*; **C**, *noisy moons*; **D**, *different density*; **E**, *anisotropic distributions*; **F**, *no structure*.', out.width='80%', fig.asp=1.2, fig.align='center', echo=F}
library(ggplot2)
library(GGally)
...
The bar plot confirms that the optimum number of clusters is three.
...
To visualize the result in a 2D scatter plot we first need to apply dimensionality reduction. We will use principal component analysis (PCA), which was described in the dimensionality reduction chapter.
...
**Solutions to exercises can be found in E2_Clustering_ExerciseSolutions.**
